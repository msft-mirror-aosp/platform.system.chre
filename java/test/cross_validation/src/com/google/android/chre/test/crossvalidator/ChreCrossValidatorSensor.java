...
public class ChreCrossValidatorSensor
    ...
    private void updateTestingSensor() {
        if (mSensor.getName().equals(mChreUsedSensorName)) {
        ...
        for (Sensor sensor : mSensorList) {
            ...
            if (sensor.getName().contains(mChreUsedSensorName)
                ...
                Log.d(TAG, "Updating test sensor to " + mChreUsedSensorName);
                mSensor = sensor;
                return;
            } else {
                Log.d(TAG, "current sensor name " + sensor.getName());
            }
        }
        Log.i(TAG, "Cannot find the same sensor on host that has name ["
                + mChreUsedSensorName + "]");
        return;
    }
