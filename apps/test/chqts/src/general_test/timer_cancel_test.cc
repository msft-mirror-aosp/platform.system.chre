...
void TimerCancelTest::startStages() {
  for (uint32_t i = 0; i < kStageCount; i++) {
    ...
    if (stage->timerId == CHRE_TIMER_INVALID) {
      EXPECT_FAIL("Unable to set timer:", &i);
    }
    ...
    if (!chreTimerCancel(stage->timerId)) {
      EXPECT_FAIL("Unable to cancel timer:", &i);
    }
    if (chreTimerCancel(stage->timerId)) {
      EXPECT_FAIL("Claimed success in second cancel:", &i);
    }
    markSuccess(i);
  }
}
...
void TimerCancelTest::setUp(uint32_t messageSize, const void * /* message */) {
  ...
  if (messageSize != 0) {
    EXPECT_FAIL("TimerCancel message expects 0 additional bytes, got ",
                &messageSize);
  }
  ...
  if (chreTimerCancel(kUnownedTimer)) {
    EXPECT_FAIL("Claimed success canceling timer we don't own");
  }
...
void TimerCancelTest::handleStageEvent(Stage *stage) {
  if (!stage->expectCallback) {
    EXPECT_FAIL("Timer didn't cancel:", &stage->stage);
  }
  ...
  if (stage->oneShot) {
    if (cancelSucceeded) {
      EXPECT_FAIL("Claimed success canceling one-shot after it fired:",
                  &stage->stage);
    }
  } else {
    if (!cancelSucceeded) {
      EXPECT_FAIL("Unable to cancel recurring timer:", &stage->stage);
    }
  }
  if (chreTimerCancel(stage->timerId)) {
    EXPECT_FAIL("Claimed success in second cancel:", &stage->stage);
  }
...
void TimerCancelTest::handleEvent(uint32_t senderInstanceId, uint16_t eventType,
  ...
  if (stage->stage >= kStageCount) {
    EXPECT_FAIL("Invalid handleEvent data:", &stage->stage);
  }
